 ...
 * 权限管理 - 角色管理 
 * */
Ext.define('ssp.basic.RolePanel', {
	...
	requires: [
        ...
        'ssp.util.selector.MenuSelectorWindow'
    ],
	initComponent : function() {
		...
		//配置角色 window
		this.menuWin = Ext.create('ssp.util.selector.MenuSelectorWindow');
			...
			items:[
		       {
				...
				},
				{
					...
					handler : function(){
						...
						this.menuWin.initMethod(r.id);
						this.menuWin.setValue(r.get('menuId'),r.get('funId'));
						this.menuWin.show();
					}
				}
			]
	...
	initEvents:function(){
		...
		this.menuWin.on('configerMenu',this.configerMenu,this);
	},
	/*
	 * 用户管理init 方法
	 * */
	...
	initColumn:function(){
		this.columns = [
			...
			{text : '角色描述',flex: 3,dataIndex : 'describle'}
		]
	},
	/*
	 *  添加角色
	...
	addRole:function(){
		...
		if(form.isValid()){
			Ext.Ajax.request({
				method: 'post',
				url:globalCtx+'/basic/RoleController/add.sdo',
				params: baseParams,
				waitTitle: '请稍等片刻',
	       		waitMsg: '正在提交...',
				scope:this,
				success: function(resp) {
					...
					if (obj.result == 'success') {
						var msg = id == ""?'新建':'修改';
						SSPApp.getApplication().msg('提示', msg+'角色[ <font>'+roleName+'</font> ]成功！');			
						this.rolePanel.reset();
						this.roleWindow.close();
					} else {
	...
	deleteRole:function(){		
		var r = this.gridPanel.getSelectionModel().getSelection()[0];
		var s = this;
		var roleStr = '删除角色[ <font color="blue" >'+r.get('roleName')+'</font> ]';
		Ext.MessageBox.confirm('提示', '确定'+roleStr+' ？', function(btn) {
			...
			Ext.Ajax.request({
				method: 'post',
				url:globalCtx+'/basic/RoleController/delete.sdo',
				params: {id: r.get('id')},
				waitTitle: '请稍等片刻',
	       		waitMsg: '正在删除...',
				scope:this,
				success: function(resp) {
					var obj = Ext.util.JSON.decode(resp.responseText);
					if (obj.result == 'success') {
						SSPApp.getApplication().msg('提示', roleStr+'成功!');
					} else {
	 ...
	 *  配置菜单
	 * */
	configerMenu:function(menuIds){
		var r = this.gridPanel.getSelectionModel().getSelection()[0];
		var s = this;
		Ext.MessageBox.confirm('提示', "确定给角色[<font color=red >"+r.get('roleName')+"</font>]配置如下菜单:</br>" +
				"<font color=red>"+this.menuWin.getNames(), function(btn) {
			if (btn != 'yes') {
				return;
			}
			Ext.Ajax.request({
				method: 'post',
				url:globalCtx+'/basic/RoleController/setMenu.sdo',
				params: {roleId: r.get('id'),menuId:menuIds==""? "":menuIds},
				waitTitle: '请稍等片刻',
	       		waitMsg: '正在提交...',
				scope:this,
				success: function(resp) {
					var obj = Ext.util.JSON.decode(resp.responseText);
					if (obj.result == 'success') {
						SSPApp.getApplication().msg('提示', '配置菜单成功');
					} else {
						Ext.Msg.alert("提示", "错误信息:"+obj.info);
					}
					s.gridPanelLoadData();
...
//		var s = this;
//		Ext.MessageBox.confirm('提示', "确定给角色[<font color=red >"+r.get('roleName')+"</font>]配置如下菜单:</br>" +
//				"<font color=red>"+this.menuWin.getNames()+
//				"</font>;<br>和如下权限：<font color=red>"+this.menuWin.getFunctionNames()+"</font>?", function(btn) {
//			if (btn != 'yes') {
//				return;
//			}
//			Ext.Ajax.request({
//				method: 'post',
//				url:globalCtx+'/basic/RoleController/setMenu.sdo',
//				params: {roleId: r.get('id'),menuId:menuIds==""? "":menuIds,funId:functionIds==""? "":functionIds},
//				waitTitle: '请稍等片刻',
//	       		waitMsg: '正在提交...',
//				scope:this,
//				success: function(resp) {
...
//		});
	...
	gridPanelLoadData:function(){
		this.roleStore.load({params:{"extLimit.start":0,"extLimit.limit":25}});
	},
	/*
	 *  刷新表格
	 * */
	...
	setBtn:function(sm){
		...
		if(sm<1){
			this.app.setModuleBtnDisable(this.toolBar,['修改角色','删除角色','配置菜单及权限']);
		}else{
			this.app.setModuleBtnDisable(this.toolBar,['新建角色']);
		}
	},
	/*
	 *  错误处理
